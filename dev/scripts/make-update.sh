...
make_main_update() {
  ...
  check_current_branch
  read -rp "Run \`git submodule foreach git checkout $BRANCH_NAME\` now? [Y/n]: "
  case "$REPLY" in
    Y|y|Yes|yes|YES|"") git submodule foreach git checkout $BRANCH_NAME ;;
  esac
  for mod in $(git submodule status | awk '{print $2}'); do
    (
      cd "$mod"
  ...
  log_cmd="git log --oneline --no-decorate main..staging"
  [[ "$($log_cmd | grep -c . )" -gt 0 ]] || {
